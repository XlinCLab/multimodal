...
corrected_frames_csv = joinpath(root_folder, "frame_numbers_corrected_with_tokens.csv")
@info "Loading corrected frames data from $corrected_frames_csv"
frames_corrected = CSV.read(corrected_frames_csv, DataFrame)
surface_fixation_csv = joinpath(root_folder, "all_trial_fixations.csv")
...
surface_gaze_csv = joinpath(root_folder, "all_trial_gazes.csv")
...
write_results_csv(surface_positions, root_folder, "all_surface_matrices.csv", "surface position matrices"; out=out)
...
write_results_csv(yolo_coordinates, root_folder, "all_yolo_coordinates.csv", "YOLO coordinates")
...
write_results_csv(image_sizes, root_folder, "image_sizes.csv", "image sizes")
...
write_results_csv(all_frame_objects, root_folder, "all_frame_objects_surfaces.csv", "surface data for recognized objects"; out=log_file)
close(log_file)
...
write_results_csv(all_trial_fixations_with_objects, root_folder, "all_trial_surfaces_fixations_with_objects.csv", "all trial surface fixations with objects"; out=log_file)
write_results_csv(all_trial_gazes_with_objects, root_folder, "all_trial_surfaces_gazes_with_objects.csv", "all trial surface gazes with objects"; out=log_file)
close(log_file)
...
write_results_csv(target_gazes, root_folder, "target_gazes_1sec.csv"; out=log_file)
write_results_csv(target_fixations, root_folder, "target_fixations_1sec.csv"; out=log_file)
close(log_file)
